...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Ala o Comedia Saiu @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '❬❗❭ Espere',
				success: 'Prontinho s2',
				error: {
					stick: 'Não Foi Possivel Fazer a Figurinha',
					Iv: 'LINK INVÁLIDO'
				},
				only: {
					group: '❌Esse comando é para ser usado em grupos❌',
					ownerG: 'Só Adm Pode Usar Isso Comedia',
					ownerB: '❌ Só Meu Dono Pode Usar Isso Arrombado ❌',
					admin: '❌Só Meu Dono Pode Usar Isso Arrombado!',
					Badmin: 'Eu Preciso Ser Adm Para Executar Esse Comando'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5513997744720@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'nsfwloli':
					...
					break*/
				case 'hilih':
					if (args.length < 1) return reply('Cade o texto loko ?')
				...
				case 'ytsearch':
					if (args.length < 1) return reply('Oque você quer procurar no youtube ? ')
				...
				case '':
					if (args.length < 1) return reply('Cade o Link vei ?')
				...
				case 'stalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Cade o Nome do Usuario?', text, {quoted: mek})
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('Oque você quer escrever?')
					teks = body.slice(7)
					reply(mess.wait)
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					if (!isOwner) return reply('Quem é você? KKKKKKKKKJ') 
				...
				case 'bc':
					if (!isOwner) return reply('Quem é Voce? KKKKKKKKKKKKJ') 
					...
					if (mentioned.length > 1) {
						teks = 'Promovido a: "Capitao Broxa"\n'
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Esse numero é privado! Não foi possivel adicionar' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque alguem para ser expulso!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Sefudeu foi de ban:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Sefudeu foi de ban : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
//AQUI NAO PRECISA MUDAR                   
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('Falha para converter em imagem')
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Pai ta On! rs')
						welkom.push(from)
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Boas Vindas ao grupo ativo com sucesso!')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
                                      break
			...
			//case 'clone':
				...
				case 'wait':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Só aceitamos foto macaco.')
					}
					break
				default:
